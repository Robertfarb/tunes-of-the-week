...
lastFmArtistCall : lastFmTunesCall
  ...
  if (requestedData === 'artists') {
    ...
    for (let i = 0; i < 25; i++) {
      artistData[i] = {
        ...
        artistImg: topArtists[i].image[2]["#text"],
      }
    }
    ...
    for (let i = 0; i < 25; i++) {
      artistData[i] = {
        ...
        url: topTunes[i].url,
        albumImage: topTunes[i].image[2]["#text"]
      }
    }
  }
  ...
  circles.enter().append("circle")
    ...
    .on("dblclick", function (d) {window.location.href = d.url, "_blank";})
    .on("mousemove", function (d) {
      return tooltip.style("top", (event.pageY - 10) + "px")
                    .style("left", (event.pageX + 10) + "px")
                    // .style("background-image", 'url(' + d.artistImg + ')');
    })
